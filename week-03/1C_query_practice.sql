...
-- Write a query against the orders table that displays the total number of distinct customers who have placed orders, by customer ID, for each country where orders have been shipped. Order by CustomerCount largest to smallest.
...
-- What are the products that we carry where we have less than 25 units on hand, but 1 or more units of them are on order? Order them by quantity on order (high to low), then by product name.
